...
```{r message=FALSE}
db.means<-get.means.ci.cosinor(f1, contrast.mean.frm="~gender",nsim=500)
db.means
...
```{r, message=FALSE}
db.means2<-get.means.ci.cosinor(fit=f2, contrast.mean.frm="~gender", nsim=500)
db.means2
```
...
```{r, message=FALSE}
db.means<-get.means.ci.cosinor(fit=f3, contrast.mean.frm="~bmi_baseline_cat|gender", nsim=500)
head(db.means)
```
...
## This code creates blank axis
p3<-ggplot.cosinor.lmer(object=f2,
                       x_str="bmi_baseline_cat_gender",
                       period=24,
                       db.means=db.means,
                       DATA=db.model)

p3<-p3 + labs(x="Time (hours)", y="HRV SDNN, (ms)")+
  scale_color_manual(values=c(
    "blue",
    "forestgreen",
    "orange",
  ...
  geom_line(aes(color=factor(GROUP, levels = c("Normal,Woman","Overweight,Woman","Obese,Woman",
                                               "Normal,Man","Overweight,Man","Obese,Man")),
                linetype=factor(GROUP, levels = c("Normal,Woman","Overweight,Woman","Obese,Woman",
                                                  "Normal,Man","Overweight,Man","Obese,Man"))))+
  theme(strip.background = element_rect(color="black", fill="white", linetype=0))+
...
p.fig2<-ggarrange(p3, p, labels=c("A","B"), ncol=1)
p.fig2
