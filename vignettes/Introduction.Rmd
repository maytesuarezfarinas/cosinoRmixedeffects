...
## this step will be updated after connect it to Github
## number of simulations will be changed to 500 after everything else looks okay (knit takes too long)
devtools::load_all()
library(cosinoRmixedeffects)
...
```{r message=FALSE}
db.means<-get.means.ci.cosinor(f1, contrast.mean.frm="~gender",nsim=100)
db.means
...
db.delta<-get.contrasts.ci.cosinor(f1,contrast.frm="~gender",nsim=100)
db.delta
...
```{r, message=FALSE}
db.means<-get.means.ci.cosinor(fit=f2, contrast..mean.frm="~T0toT14", nsim=100)
db.means
```
...
```{r, message=FALSE}
db.means2<-get.means.ci.cosinor(fit=f2, contrast.mean.frm="~gender", nsim=100)
db.means2
```
...
```{r, message=FALSE, fig.show='hide'}
db.delta<-get.contrasts.ci.cosinor(fit=f2,contrast.frm="~T0toT14", nsim=100)
db.delta
```
...
```{r, message=FALSE}
db.means<-get.means.ci.cosinor(fit=f3, contrast.mean.frm="~bmi_baseline_cat|gender", nsim=100)
head(db.means)
```
...
```{r, message=FALSE, fig.show='hide'}
db.delta<-get.contrasts.ci.cosinor(fit=f3,contrast.frm="~bmi_baseline_cat|gender", nsim=100)
head(db.delta)
```
