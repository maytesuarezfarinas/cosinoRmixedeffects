...
just.get.contrasts.cosinor <- function (fit, contrast.frm, output = "pairwise.diff", ...)
{
  ...
  out <- c(pars.raw.mesor, amp, acr, PeakTime)
  if (output == "pairwise.diff") {
    out <- c(get.pairwise.diff(pars.raw.mesor), get.pairwise.diff(amp),
             get.pairwise.diff(acr), get.pairwise.diff(PeakTime))
  }
  return(out)
}
