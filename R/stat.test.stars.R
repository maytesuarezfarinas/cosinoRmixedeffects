...
#'
stat.test.stars <- function(db.delta, contrast.frm){
  require(rstatix)
  ...
  stat.test <- stat.test %>% add_significance(cutpoints = c(0, 1e-04, 0.001, 0.01, 0.05, 0.1, 1),
                                            symbols = c("****", "***", "**", "*", "+", "ns"))
  ...
  stat.test<-do.call("rbind.data.frame", lapply(c("MESOR", "Amplitude", "Acrophase"), function(L){
    ...
    db.temp2<-add_y_position(db.temp, formula=as.formula(paste0("`97.5 %`",contrast.frm)), # need to specify variable here
